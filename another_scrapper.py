    ...
    def get_links(self):
        ...
        try:
            with Stealth().use_sync(sync_playwright()) as playwright:
                ...
                url= re.search(r'^(.*/maps/)',self.url)
                if url:
                    url=url.group(1)
                # page.goto(f'https://yandex.by/maps/?text={name}')
                page.goto(url+'?text='+self.name)
                ...
                # stores_lists = page.locator('a[href*="/maps/org/"]')
                stores_lists = page.locator('a[href*="/reviews/"]')


                # sup_url= re.search(r'^(.*)?/maps',url)
                # if sup_url:
                #     sup_url=sup_url.group(1)
                ...
                for link in links:
                    ...
                    if href:
                        if href.startswith('/'):
                            ...
                            new_url=sup_url+'/'+href
                        # if 'reviews' in href:
                        # new_url = 'https://yandex.by'+href if href.startswith('/') else 'https://yandex.by/'+href
...
# links= get_links('Соседи',url=url)
# print(len(links))


    def collect_reviews(self,links: set, path_to_file):
        num_links = len(links)
        link_cnt = 0 
        if sys.platform == 'win32':
            if not isinstance(asyncio.get_event_loop_policy(), asyncio.WindowsProactorEventLoopPolicy):
        ...
        with Stealth().use_sync(sync_playwright()) as playwright:
            ...
            for i, link in enumerate(links, 1):
                # page.wait_for_timeout(np.random.randint(2000,10001))
                page.wait_for_timeout(40000)
                
                page.goto(link)
                page.wait_for_selector('.business-review-view')
                page.wait_for_selector('h2.card-section-header__title._wide')
                cnt= page.locator('h2.card-section-header__title._wide').first.inner_text()
                ...
                for _ in range(40):
                    ...
                    info=page.locator('span.spoiler-view__text-container').all()
            
                    for text in info:
                        if text not in all_reviews:
                            with open(path_to_file, 'a', encoding='utf-8') as f:

                                f.write(text.inner_text().replace('\n','')+'\n')
                                all_reviews.append(text.inner_text())


                # print(f'записано отзывов: {len(info)}/{cnt}')
                curr_cnt=len(all_reviews)

                if curr_cnt==before_cnt:
                    # print('Конец или не получается')
                    break
                before_cnt=curr_cnt
                
                # print(f'обработана ссылка: {link}')
                # print('--------------------------')
                percentage= int((i/num_links)*100)
                if i==num_links:
                    browser.close()
                    yield percentage
                else:
...
links_cnt = ['https://yandex.by/maps/org/znwr/231467783292/reviews/', 'https://yandex.by/maps/org/znwr/151170805561/reviews/', 'https://yandex.by/maps/org/znwr/226274225984/reviews/']
